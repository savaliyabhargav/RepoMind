...
public class RepoOverviewService {
    ...
    public FileExplainResponse overview(UUID repoId, String providerInput, boolean refresh) {
        ...
        List<FileNodeSummary> files = ingestionServiceClient.listFiles(canonicalRepoId, "FILE");
        if (files.isEmpty()) {
        ...
        String fingerprint = ExplainCacheService.sha256(
                files.stream().map(FileNodeSummary::path).collect(Collectors.joining("\n")));

        if (!refresh) {
            var cached = explainCacheService.find(canonicalRepoId, OVERVIEW_PATH, fingerprint);
            if (cached.isPresent()) {
                var entry = cached.get();
                log.info("[overview] cache hit repo={} fingerprint={}", repo.name(), fingerprint.substring(0, 12));
        ...
        explainCacheService.save(canonicalRepoId, OVERVIEW_PATH, fingerprint,
    ...
    private String buildUserPrompt(RepoSummary repo, List<FileNodeSummary> files) {
        StringBuilder tree = new StringBuilder();
        int lines = 0;
        for (FileNodeSummary node : files) {
            tree.append(node.path());
            if (node.roleSummary() != null && !node.roleSummary().isBlank()) {
                tree.append(" — ").append(node.roleSummary());
            }
            tree.append('\n');
        ...
        return """
                ...
                ---
                %s---
                """.formatted(repo.owner(), repo.name(), files.size(), tree);
    }

    private FileExplainResponse toResponse(RepoSummary repo, DiagramPayload payload) {
