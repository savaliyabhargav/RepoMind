package com.repomind.explain.github;
...
public class GitHubContentClient {
    ...
    public Optional<String> fetchFileContent(String owner, String name, String defaultBranch, String githubToken, String path) {
        if (owner == null || name == null || path == null || path.isBlank()) {
            return Optional.empty();
        }

        String branch = Optional.ofNullable(defaultBranch)
                .filter(v -> !v.isBlank())
                .orElse("main");
        ...
        try {
            Map<String, Object> response = webClient.get()
                    ...
                    .headers(headers -> applyAuth(headers, githubToken))
                    .retrieve()
            ...
            if (!downloadUrl.isBlank()) {
                return fetchRaw(downloadUrl, githubToken);
            }
        } catch (Exception ex) {
            log.debug("Failed to fetch GitHub content for {}/{} path={}: {}", owner, name, path, ex.getMessage());
        }
        return Optional.empty();
    }

    private Optional<String> fetchRaw(String downloadUrl, String githubToken) {
        try {
    ...
    private void applyAuth(HttpHeaders headers, String githubToken) {
