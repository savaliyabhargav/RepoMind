...
import com.repomind.analysis.domain.AiCallLog;
import com.repomind.analysis.domain.AiCallLogRepository;
import com.repomind.analysis.domain.Analysis;
import com.repomind.analysis.domain.AnalysisRepository;
import com.repomind.analysis.domain.AnalysisStage;
import com.repomind.analysis.domain.AnalysisStageRepository;
import com.repomind.analysis.auth.AuthServiceClient;
import com.repomind.analysis.ingestion.IngestionServiceClient;
import com.repomind.analysis.ingestion.IngestionServiceClient.FileNodeSummary;
import com.repomind.analysis.ingestion.IngestionServiceClient.RepoSummary;
import com.repomind.analysis.ai.AiProviderRouter;
...
import com.repomind.analysis.RetrievalService;
...
public class AnalysisPipelineService {
    ...
    public AnalysisResponse runPipeline(UUID repoId, UUID userId, String aiProviderInput) {
        ...
        List<FileNodeSummary> repoNodes = ingestionServiceClient.listFiles(canonicalRepoId);
        List<FileNodeSummary> repoFiles = repoNodes.stream()
                .filter(node -> "FILE".equalsIgnoreCase(node.type()))
                .toList();
        List<FileNodeSummary> analyzableFiles = repoFiles.stream()
        ...
        try {
            ...
            if (deepPassEnabled) {
                pipelineResult.put("DEEP_PASS", buildDeepPass(analysis, repo.owner(), repo.name(), ctx));
            }

            if (remainingMs(ctx) > MIN_REMAINING_MS_FOR_QUALITY_SYNTHESIS) {
                pipelineResult.put("QUALITY_SYNTHESIS", buildQualitySynthesis(analysis, repo.owner(), repo.name(), pipelineResult));
            }
    ...
    private StageExecutionResult runStage(
            ...
            Analysis analysis,
            RepoSummary repo,
            List<FileNodeSummary> repoNodes,
            List<FileNodeSummary> repoFiles,
            List<FileNodeSummary> analyzableFiles,
    ...
    private StageExecutionResult buildOverview(Analysis analysis, RepoSummary repo, List<FileNodeSummary> nodes, List<FileNodeSummary> files) {
        ...
        String entryPoint = files.stream()
                .map(FileNodeSummary::path)
                .sorted(Comparator.comparingInt(this::entrypointPriority).reversed())
                .filter(path -> entrypointPriority(path) > 0)
                .findFirst()
                .orElse(files.isEmpty() ? "" : files.get(0).path());

        Map<String, Object> overview = new LinkedHashMap<>();
        overview.put("repoName", repo.name());
        overview.put("owner", repo.owner());
        overview.put("defaultBranch", repo.defaultBranch());
    ...
    private StageExecutionResult buildArchitecture(Analysis analysis, List<FileNodeSummary> nodes) {
    ...
    private StageExecutionResult buildModuleMap(Analysis analysis, List<FileNodeSummary> files, PipelineContext ctx) {
        int moduleLimit = switch (ctx.qualityProfile()) {
            case DEEP -> 45;
            case BALANCED -> 35;
            case FAST -> 25;
        };
        List<FileNodeSummary> moduleCandidates = files.stream()
                .filter(this::isModuleCandidate)
                .sorted(Comparator.comparingInt((FileNodeSummary file) -> modulePathPriority(file.path())).reversed())
                .toList();
        ...
        List<Map<String, Object>> modules = moduleCandidates.stream()
                ...
                .map(file -> {
                    ...
                    module.put("path", file.path());
                    module.put("language", Optional.ofNullable(file.language()).orElseGet(() -> inferLanguage(file.path())));
                    module.put("role", inferRole(file.path()));
                    return module;
                })
                .toList();
    ...
    private StageExecutionResult buildDataFlow(Analysis analysis, List<FileNodeSummary> files, PipelineContext ctx) {
        ...
        List<FileNodeSummary> runtimeFlowCandidates = files.stream()
                .filter(this::isRuntimeFlowCandidate)
                .toList();
        List<FileNodeSummary> dataAccessCandidates = files.stream()
    ...
    private StageExecutionResult buildRiskScan(Analysis analysis, List<FileNodeSummary> files, PipelineContext ctx) {
        int riskLimit = switch (ctx.qualityProfile()) {
            case DEEP -> 12;
            case BALANCED -> 8;
            case FAST -> 6;
        };
        List<FileNodeSummary> riskCandidates = files.stream()
    ...
    private StageExecutionResult annotateFiles(Analysis analysis, List<FileNodeSummary> files, PipelineContext ctx) {
        List<FileNodeSummary> candidates = files.stream()
                .filter(file -> Optional.ofNullable(file.sizeBytes()).orElse(0L) <= MAX_ANNOTATION_FILE_SIZE_BYTES)
                .toList();
        ...
        Map<String, String> heuristicSummaries = candidates.stream()
                .collect(Collectors.toMap(FileNodeSummary::path, file -> inferRole(file.path())));

        for (FileNodeSummary file : candidates) {
            String summary = heuristicSummaries.get(file.path());
            if (Objects.equals(normalizeForCompare(file.roleSummary()), normalizeForCompare(summary))) {
                unchanged++;
                continue;
            }
            ingestionServiceClient.updateRoleSummary(file.id(), summary);
            updated++;
        }
        ...
        if (canUseAiEnrichment) {
            List<FileNodeSummary> aiTargets = candidates.stream()
                    .filter(file -> GENERIC_ROLE.equals(heuristicSummaries.get(file.path())))
                    .sorted(Comparator.comparingInt((FileNodeSummary file) -> dataFlowPathPriority(file.path())).reversed())
            ...
            for (int i = 0; i < aiTargets.size(); i += 20) {
                ...
                List<FileNodeSummary> batch = aiTargets.subList(i, Math.min(i + 20, aiTargets.size()));
                Map<String, String> summaries = generateFileSummaries(analysis, batch);
                for (FileNodeSummary file : batch) {
                    String summary = summaries.getOrDefault(file.path(), heuristicSummaries.get(file.path()));
                    if (summary == null || summary.isBlank() || GENERIC_ROLE.equalsIgnoreCase(summary.trim())) {
                        summary = heuristicSummaries.get(file.path());
                    }
                    if (!Objects.equals(normalizeForCompare(file.roleSummary()), normalizeForCompare(summary))) {
                        ingestionServiceClient.updateRoleSummary(file.id(), summary);
                    }
    ...
    private Map<String, Object> buildDeepPass(Analysis analysis, String repoOwner, String repoName, PipelineContext ctx) {
        ...
        try {
            ...
            DeepSynthesisOutput synthesis = buildDeepSynthesis(analysis, repoOwner, repoName, flowSearch, architectureSearch, ctx);
    ...
    private Map<String, Object> buildQualitySynthesis(Analysis analysis, String repoOwner, String repoName, Map<String, Object> pipelineResult) {
    ...
    private List<String> filterByAnyKeyword(List<FileNodeSummary> files, List<String> keywords, int limit, boolean excludeInfraPaths) {
    ...
    private boolean isModuleCandidate(FileNodeSummary file) {
    ...
    private boolean isRuntimeFlowCandidate(FileNodeSummary file) {
        String lower = file.path().toLowerCase(Locale.ROOT);
        if (isInfraPath(lower) || isDocumentationPath(lower) || isTestPath(lower) || isBuildManifestPath(lower)) {
            return false;
        }
    ...
    private boolean isDataAccessCandidate(FileNodeSummary file) {
        String lower = file.path().toLowerCase(Locale.ROOT);
        if (isInfraPath(lower) || isDocumentationPath(lower) || isTestPath(lower) || isBuildManifestPath(lower)) {
            return false;
        }
    ...
    private boolean isRiskCandidate(FileNodeSummary file) {
        String lower = file.path().toLowerCase(Locale.ROOT);
    ...
    private List<String> highestPriorityPaths(List<FileNodeSummary> files, int limit) {
        return files.stream()
                .map(FileNodeSummary::path)
                .sorted(Comparator.comparingInt(this::dataFlowPathPriority).reversed())
                .limit(limit)
    ...
    private boolean isAnalyzableTextFile(FileNodeSummary file) {
        long size = Optional.ofNullable(file.sizeBytes()).orElse(0L);
        if (size == 0L) {
            return true;
        }
    ...
    private String getExtension(FileNodeSummary file) {
        return getExtensionFromPath(file.path());
    }
    ...
    private String buildOverviewHeuristicSummary(
        ...
        return "Repository `" + repo.name() + "` has " + fileCount + " files across "
    ...
    private Map<String, Object> buildRiskEntry(FileNodeSummary file) {
        String lower = file.path().toLowerCase(Locale.ROOT);
        long sizeBytes = Optional.ofNullable(file.sizeBytes()).orElse(0L);
        int score = 0;
    ...
    private Map<String, String> generateFileSummaries(Analysis analysis, List<FileNodeSummary> batch) {
        String systemPrompt = """
        ...
        String userPrompt = "Files:\n" + batch.stream()
                .map(file -> "- " + file.path() + " | lang=" + Optional.ofNullable(file.language()).orElse("unknown"))
                .collect(Collectors.joining("\n"));
        try {
            ...
            return batch.stream().collect(Collectors.toMap(FileNodeSummary::path, file -> inferRole(file.path())));
        }
    }
